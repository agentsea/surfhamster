...
class SurfHamsterConfig(BaseModel):
    pass


class SurfHamster(TaskAgent):
    ...
    def solve_task(
        ...
        # UPD: We are using our tool wrapper instead of barebone device here
        semdesk = SemanticDesktop(task, device)
        ...
        # UPD: Note that we don't only use our SemanticDesktop tool here, 
        # but we also remove some tools we don't need our agent to use
        ...
        # UPD: We replace the call of the device's json_schema() with a reference to the tools,
        ...
        for i in range(max_steps):
            ...
            try:
                # UPD: The action is taken upon the SemanticDesktop tool
    ...
    def take_action(
        self,
        semdesk: SemanticDesktop, # UPD: We use our SemanticDesktop tool here
        task: Task,
        thread: RoleThread,
        ...
        try:
            ...
            # UPD: SemanticDesktop tool doesn't have take_screenshot() method, 
            # so we use the device's method instead
            ...
            # UPD: SemanticDesktop tool doesn't have mouse_coordinates() method, 
            # so we use the device's method instead
            ...
            # UPD: We use the SemanticDesktop tool to find the action
            ...
            # UPD: We use the SemanticDesktop tool to take the selected action
            try:
            ...
            # UPD: We use the SemanticDesktop tool to record the action
            task.record_action(
    ...
    @classmethod
    def config_type(cls) -> Type[SurfHamsterConfig]:
        """Type of config

        Returns:
            Type[DinoConfig]: Config type
        """
        return SurfHamsterConfig

    @classmethod
    def from_config(cls, config: SurfHamsterConfig) -> "SurfHamster":
        ...
        Returns:
            SurfHamster: The agent
        """
        return SurfHamster()

    @classmethod
    def default(cls) -> "SurfHamster":
